...
let currentLogFileName = createLogFile("service-ss01-subarea");
...
export async function getListByIdArea(id_area: number) {
  try {
    const url = `${SS01_URL}${BASE_PATH}/list/area/${id_area}`;
    ...
    logFormat.message = "Success Get List Subarea By IdArea";
    logFormat.status = response.status;
    logFormat.data = response.data;
    const data_log = JSON.stringify(logFormat);
    writeLogToFile(data_log, currentLogFileName);
