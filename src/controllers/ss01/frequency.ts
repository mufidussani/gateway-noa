...
import * as FrequencyAPIRepo from "../../repo/api/ss01/frequency";
...
let currentLogFileName = createLogFile("ss01-frequency");
...
export async function create(req: Request, res: Response, next: any) {
  ...
  try {
    ...
    if (!verify) {
      logFormat.message = "Error Create Frequency";
    ...
    logFormat.message = "Processing Create Frequency";
...
export async function getAll(req: Request, res: Response, next: any) {
  try {
    ...
    const listFrequency = await FrequencyAPIRepo.getAll(
      searchValue,
      limit,
      offset
    );
    ...
    if (!listFrequency) {
      throw new Error("Error Get List Frequency service ss01");
    }

    return res.status(200).json({
      message: "Success",
      data: listFrequency.data.data,
      total: listFrequency.data.total,
    });
  } catch (err: any) {
    logFormat.message = "Error GetAll Frequency";
    logFormat.status = 500;
    logFormat.data = { err: err.toString() };
    const data_log = JSON.stringify(logFormat);
