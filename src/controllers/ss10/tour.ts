...
import * as TourAPIRepo from "../../repo/api/ss10/tour";
...
let currentLogFileName = createLogFile("ss10-tour");
...
export async function create(req: Request, res: Response, next: any) {
  ...
  try {
    ...
    logFormat.message = "Processing Create Tour";
...
export async function getByIDClient(req: Request, res: Response, next: any) {
  try {
    if (!req.params.id_client) {
      logFormat.message = "Error GetTour By ID Client";
    ...
    const dataTour = await TourAPIRepo.getByIDClient(
      searchValue,
      limit,
      offset,
      startDate,
    ...
    if (!dataTour) {
      throw new Error("Error GetTour By ID Client service ss10");
    }

    return res.status(200).json({
      message: "Success",
      data: dataTour.data.data,
      total: dataTour.data.total,
    });
  } catch (err: any) {
    logFormat.message = "Error GetTour By ID Client";
    logFormat.status = 500;
...
export async function getByIDProject(req: Request, res: Response, next: any) {
  try {
    if (!req.params.id_project) {
      logFormat.message = "Error GetTour By ID Project";
      logFormat.status = 404;
      logFormat.data = "Not Found";
      const data_log = JSON.stringify(logFormat);
      writeLogToFile(data_log, currentLogFileName);
    ...
    const dataTour = await TourAPIRepo.getByIDProject(
      searchValue,
      limit,
      offset,
      startDate,
    ...
    if (!dataTour) {
      throw new Error("Error GetTour By ID Project service ss10");
    }
