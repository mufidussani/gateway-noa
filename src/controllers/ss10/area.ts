...
import * as AreaAPIRepo from "../../repo/api/ss10/area";
...
let currentLogFileName = createLogFile("ss10-area");
...
export async function getByIdProject(req: Request, res: Response, next: any) {
  try {
    ...
    if (!dataArea) {
      throw new Error("Error GetArea By IdProject service ss10");
    }
    ...
    logFormat.message = "Error GeArea By IdProject";
    logFormat.status = 500;
    logFormat.data = err!;
    const data_log = JSON.stringify(logFormat);
...
export async function getByID(req: Request, res: Response, next: any) {
  try {
    ...
    if (!detailArea) {
      throw new Error("Error GetArea By Id service ss10");
    }
