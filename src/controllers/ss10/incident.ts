...
import * as IncidentAPIRepo from "../../repo/api/ss10/incident";
...
let currentLogFileName = createLogFile("ss10-incident");
...
export async function create(req: Request, res: Response, next: any) {
  ...
  try {
    ...
    if (!verify) {
      logFormat.message = "Error Create Incident";
    ...
    logFormat.message = "Processing Create Incident";
...
export async function getByIDClient(req: Request, res: Response, next: any) {
  try {
    if (!req.params.id_client) {
      logFormat.message = "Error GetIncident By ID Client";
    ...
    const dataIncident = await IncidentAPIRepo.getByIDClient(
      searchValue,
      limit,
      offset,
      startDate,
    ...
    if (!dataIncident) {
      throw new Error("Error GetIncident By ID Client service ss10");
    }

    return res.status(200).json({
      message: "Success",
      data: dataIncident.data.data,
      total: dataIncident.data.total,
    });
  } catch (err: any) {
    logFormat.message = "Error GetIncident By ID Client";
    logFormat.status = 500;
...
export async function getByIDProject(req: Request, res: Response, next: any) {
  try {
    if (!req.params.id_project) {
      logFormat.message = "Error GetIncident By ID Project";
      logFormat.status = 404;
      logFormat.data = "Not Found";
      const data_log = JSON.stringify(logFormat);
      writeLogToFile(data_log, currentLogFileName);
    ...
    const dataIncident = await IncidentAPIRepo.getByIDProject(
      searchValue,
      limit,
      offset,
      startDate,
    ...
    if (!dataIncident) {
      throw new Error("Error GetIncident By ID Project service ss10");
    }
